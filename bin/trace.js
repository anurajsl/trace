...
${c.bold}CI/CD:${c.reset}
  ...
  ${c.cyan}trace metrics --json${c.reset}            JSON report for dashboards + org reporting
...
${c.bold}Automation:${c.reset}
  ...
  ${c.cyan}trace activate${c.reset}                  Install all git hooks (pre-commit + post-commit + post-checkout)
  ${c.cyan}trace activate status${c.reset}            Check which hooks are active
  ${c.cyan}trace deactivate${c.reset}                 Remove all TRACE git hooks
  ${c.cyan}trace acknowledge${c.reset}                Mark drift as intentional (stops recurring warnings)
  ${c.cyan}trace acknowledge --list${c.reset}          Show all acknowledged drift
...
async function main() {
  switch (command) {
    ...
    case 'acknowledge':
    case 'ack': {
      const { runAcknowledge } = await import('../src/acknowledge.js');
      runAcknowledge(args);
      break;
    }
    case 'activate': {
      const { runActivate } = await import('../src/activate.js');
      runActivate(args);
      break;
    }
    case 'deactivate': {
      const { runActivate } = await import('../src/activate.js');
      runActivate(['deactivate']);
      break;
    }
    case 'license': {
