...
export async function runGateStart() {
  ...
  printHeader(`Start Gate — ${config.project?.name || 'Project'}`);
  ...
  if (gatePass) {
    ...
    try {
      ...
      const ctxPath = generateSessionContext(root, config);
      printPass('Session context generated (.trace/AI_CONTEXT.md)');
    } catch (e) {
      // Non-critical
    }
  } else {
