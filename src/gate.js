...
export async function runGateStart(args = []) {
  ...
  if (!root) {
    if (!args.includes('--quiet')) console.log(`${c.red}No trace.yaml found.${c.reset} Run ${c.cyan}trace init${c.reset} first.`);
    process.exit(1);
  }
  ...
  if (!quiet) printHeader(`Start Gate — ${config.project?.name || 'Project'}${scope ? ` [scope: ${scope}]` : ''}`);
  ...
  // Quiet mode: just regenerate context, no output (used by git hooks)
  if (quiet) {
    if (gatePass || onFail !== 'block') {
      try {
        const { generateSessionContext } = await import('./observe.js');
        generateSessionContext(root, config, scope);
      } catch {}
    }
    return;
  }
...
export async function runGateEnd() {
  ...
  // Planning reconciliation (skip if planning.enabled is false)
  const planningEnabled = config.planning?.enabled !== false; // default true
  if (planningEnabled) {
    try {
  ...
  try {
    ...
    const { loadAcknowledgedDrift } = await import('./acknowledge.js');
    ...
    if (!syncResult.skipped) {
      ...
      // Filter acknowledged drift from violations
      const filteredViolations = [];
      let ackTotal = 0;
      for (const v of (syncResult.violations || [])) {
        const realStale = v.staleConsumers.filter(sc => !acknowledged.has(`${v.anchor}:${sc}`));
        ackTotal += v.staleConsumers.length - realStale.length;
        if (realStale.length > 0) {
          filteredViolations.push({ ...v, staleConsumers: realStale });
        }
      }

      if (filteredViolations.length === 0) {
        const ackMsg = ackTotal > 0 ? ` ${c.dim}(${ackTotal} acknowledged)${c.reset}` : '';
        printPass(`All modified anchors have updated consumers${ackMsg}`);
      } else {
        for (const v of filteredViolations) {
          printFail(`Anchor "${v.anchor}" changed but ${v.staleConsumers.length} consumer(s) not updated:`);
          for (const sc of v.staleConsumers) {
            printFail(`  → ${sc}`);
          }
        }
        if (ackTotal > 0) printInfo(`  ${c.dim}(${ackTotal} acknowledged drift skipped)${c.reset}`);
        gatePass = false;
        gateFailures.push({ type: 'consumer_sync', detail: `${filteredViolations.length} anchor(s) with stale consumers` });
      }
    }
  } catch (e) {
  ...
  if (gatePass) {
    ...
    try {
      const cpPath2 = path.join(root, '.trace/LIVE_CHECKPOINT.yaml');
      const statePath = path.join(root, config.artifacts?.trace_state || '.trace/TRACE_STATE.yaml');
      const st = loadYaml(statePath) || {};
      const anchors = config.anchors || [];
      const totalConsumers = anchors.reduce((s, a) => s + (a.consumers || []).length, 0);
      ...
      console.log(`  Anchors: ${anchors.length}  Consumers: ${totalConsumers}  Score: ${sc}${score}/100${c.reset}`);
      console.log();
    } catch {}
  } else {
    if (onFail === 'block') {
