 ...
 */
export function generateSessionContext(root, config) {
  const planPath = path.join(root, '.trace/PLAN.yaml');
  ...
  content += `_Auto-generated at ${getTimestamp()} — do not edit manually_\n\n`;
  ...
  // Relevant anchors — only those touched by in-progress items
  const anchors = config.anchors || [];
  if (anchors.length > 0) {
    content += `## Relevant Anchors\n\n`;
