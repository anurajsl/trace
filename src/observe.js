...
export function generateSessionContext(root, config, scope = null) {
  ...
  // Policies — injected from trace.yaml and/or org config
  const policies = config.policies || {};
  const policyCategories = Object.keys(policies);
  if (policyCategories.length > 0) {
    content += `## Policies\n\n`;
    content += `**These rules are mandatory. Follow them in every action.**\n\n`;
    for (const category of policyCategories) {
      content += `### ${category.charAt(0).toUpperCase() + category.slice(1)}\n`;
      for (const rule of policies[category]) {
        content += `- ${rule}\n`;
      }
      content += `\n`;
    }
