...
export async function runScan() {
  ...
  // Phase 6: Git co-change analysis (if git history exists)
  const gitDir = path.join(projectDir, '.git');
  if (fs.existsSync(gitDir)) {
    try {
      const gitPairs = detectGitCoChanges(projectDir, anchorCandidates);
      if (gitPairs.length > 0) {
        console.log(`\n  ${c.bold}Git co-change patterns:${c.reset}\n`);
        for (const p of gitPairs.slice(0, 5)) {
          console.log(`    ${c.cyan}↔${c.reset} ${p.fileA} ↔ ${p.fileB} ${c.dim}(${p.pct}% co-change, ${p.commits} commits)${c.reset}`);
        }
      }
    } catch {}
  }

  // Phase 7: Generate trace.yaml
 ...
 * Lightweight: only analyzes last 50 commits.
 */
function detectGitCoChanges(root, anchors) {
  const anchorFiles = new Set(anchors.map(a => a.file));
  const coChanges = {};

  try {
    // Get last 50 commits with changed files
    const log = execSync('git log --name-only --pretty=format:"---" -50', {
      cwd: root, encoding: 'utf8', timeout: 5000,
    });

    const commits = log.split('---').filter(c => c.trim());
    const fileCommits = {}; // file -> set of commit indices

    commits.forEach((commit, idx) => {
      const files = commit.trim().split('\n').filter(f => f.trim() && !f.startsWith('.trace'));
      for (const file of files) {
        if (!fileCommits[file]) fileCommits[file] = new Set();
        fileCommits[file].add(idx);
      }
    });

    // Find pairs that co-change frequently (at least one must be an anchor)
    const anchorEntries = Object.entries(fileCommits).filter(([f]) => anchorFiles.has(f));
    const results = [];

    for (const [anchorFile, anchorCommits] of anchorEntries) {
      for (const [otherFile, otherCommits] of Object.entries(fileCommits)) {
        if (otherFile === anchorFile) continue;
        const intersection = new Set([...anchorCommits].filter(c => otherCommits.has(c)));
        if (intersection.size >= 3) {
          const pct = Math.round(intersection.size / anchorCommits.size * 100);
          if (pct >= 50) {
            results.push({ fileA: anchorFile, fileB: otherFile, pct, commits: intersection.size });
          }
        }
      }
    }

    return results.sort((a, b) => b.pct - a.pct).slice(0, 10);
  } catch {
    return [];
  }
}
